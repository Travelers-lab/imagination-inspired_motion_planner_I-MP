...
class Robot:
    ...
    def load_environment(self, environment):
        ...
        for i in range(len(environment)):
            self.object.append(self.bullet_client.loadURDF(fileName=join(self.description_file, f'simulation_table/cylinder_chamfered.urdf'),
                                    basePosition=np.array([environment[i][0], environment[i][1], 1.15]),
                                    baseOrientation=[0, 0, 0, 1],
                                    flags=self.bullet_client.URDF_ENABLE_CACHED_GRAPHICS_SHAPES,
                                    useFixedBase=int(environment[i][2])))
